...
exports.createCourse = async (req, res) => {
  ...
  try {
    ...
    const requestTimestamp = parseInt(req.body.timestamp || req.query.timestamp, 10);
    const currentTimestamp = Date.now();

    console.log("Request Timestamp:", requestTimestamp);
    console.log("Current Timestamp:", currentTimestamp);

    if (currentTimestamp - requestTimestamp > MAX_DELAY) {
    return res.status(400).json({
      success: false,
      message: "Stale request - Request time exceeded allowed delay."
    });
   }
    ...
    console.log("Consoling the values->",courseName)
